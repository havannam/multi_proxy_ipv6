...
install_3proxy() {
    ...
    mkdir -p /3proxy
    cd /3proxy
    URL="https://github.com/havannam/multi_proxy_ipv6/raw/main/3proxy-0.9.4.tar.gz"
    wget -qO- $URL | bsdtar -xvf-
    cd 3proxy-0.9.4
    make -f Makefile.Linux
    mkdir -p /usr/local/etc/3proxy/{bin,logs,stat}
    mv /3proxy/3proxy-0.9.4/bin/3proxy /usr/local/etc/3proxy/bin/
    wget https://raw.githubusercontent.com/havannam/multi_proxy_ipv6/main/3proxy.service-Centos8 --output-document=/3proxy/3proxy-0.9.4/scripts/3proxy.service2
...
chmod +x $WORKDIR/v62.sh /etc/rc.local
